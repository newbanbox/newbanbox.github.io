...
def download_nes_files(base_url, nes_paths, download_folder="nes_files"):
    """
    批量下载NES文件
    """
    # 创建下载文件夹
    ...
    for i, nes_path in enumerate(nes_paths):
        # 构建完整URL
        full_url = urljoin(base_url, nes_path)
        
        # 从路径中提取文件名
        filename = os.path.basename(nes_path)
        filepath = os.path.join(download_folder, filename)
        
        print(f"正在下载 ({i+1}/{len(nes_paths)}): {filename}")
...
def generate_download_links(base_url, nes_paths):
    """
    生成完整的下载链接列表
    """
    download_links = []
    
    for nes_path in nes_paths:
        full_url = urljoin(base_url, nes_path)
        filename = os.path.basename(nes_path)
...
def main():
    ...
    # 基础URL
    base_url = "https://game.xugaoyi.com/"
    
    # 提取所有.nes路径
    nes_paths = extract_nes_paths(text)
    
    print(f"找到 {len(nes_paths)} 个NES文件路径")
    print("=" * 50)
    
    # 生成下载链接
    download_links = generate_download_links(base_url, nes_paths)
    ...
    # 询问用户是否要下载文件
    choice = input("是否要批量下载这些文件? (y/n): ").lower()
    
    if choice == 'y':
        # 批量下载文件
        downloaded, failed = download_nes_files(base_url, nes_paths)
